...
from node_classification.graph_embeddings.sage_torch import GraphSAGEEmbedder
...
def dimensionality_reduction(node_emb_technique: str, model_dir, train_df, node_embedding_size, lab, edge_path=None,
    ...
    if node_emb_technique == "node2vec":
        ...
        sage = GraphSAGEEmbedder(in_channels=in_channels, hidden_channels=node_embedding_size, out_channels=2,
                                 num_hidden_layers=len(sizes), edg_dir=edge_path, train_df=train_df, features=features_dict)
        mapper, inv_map = sage.create_mappers()
        graph = sage.create_graph(mapper, inv_map)
        x = graph.x.squeeze().to(device)
        y = graph.y.squeeze().to(device)
        sage = sage.to(device)

        train_loader = NeighborSampler(graph.edge_index, sizes=sizes, batch_size=batch_size, shuffle=True)

        optimizer = torch.optim.Adam(lr=.003, params=sage.parameters())
        for i in range(20):
            print(i)
        ...
        sage.eval()
        with torch.no_grad():
            e2 = sage.inference(x, train_loader)
            e4 = sage.inference(x, train_loader)
